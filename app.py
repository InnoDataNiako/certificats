...
# Chemins
image_path = "certificat-im.jpg"
pdf_path = "certificat.pdf"

# Fonction pour créer un lien de téléchargement pour le PDF
def create_download_link(file_path, download_name="Certificat_Anglais_NIAKO_KEBE.pdf"):
    with open(file_path, "rb") as f:
        base64_pdf = base64.b64encode(f.read()).decode()
    return f'<a href="data:application/pdf;base64,{base64_pdf}" download="{download_name}" class="download-btn">📄 Télécharger le Certificat PDF Original</a>'
...
try:
    st.image(image_path, width=700, caption="Certificat d'anglais - NIAKO KEBE", use_column_width=False)
except FileNotFoundError:
    st.error("❌ Fichier d'image non trouvé ! Assurez-vous que 'certificat-im.jpg' est dans le même dossier que l'application.")

# Bouton de téléchargement PDF
try:
    st.markdown(create_download_link(pdf_path), unsafe_allow_html=True)
except FileNotFoundError:
    st.warning("⚠️ Fichier PDF non trouvé ! Le téléchargement ne sera pas disponible.")
